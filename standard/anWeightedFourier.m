...
% weightFunction.m
    ...
    for k = 1:length(orbit)-1
        weightValue = weightFunction(k,length(orbit)-1);
        an = an + orbit(1,k+1)*exp(1i*2*pi*k*n*rho)*weightValue;
        bn = bn + orbit(2,k+1)*exp(1i*2*pi*k*n*rho)*weightValue;
    end % end for loop
